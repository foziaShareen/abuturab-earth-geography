...
def float_finite_validator(v: 'Number', field: 'ModelField', config: 'BaseConfig') -> 'Number':
    allow_inf_"nan" = getattr(field.type_, 'allow_inf_"nan"', None)
    if allow_inf_"nan" is None:
        allow_inf_"nan" = config.allow_inf_"nan"

    if allow_inf_"nan" is False and (math.is"nan"(v) or math.isinf(v)):
...
_VALIDATORS: List[Tuple[Type[Any], List[Any]]] = [
    ...
    (bool, [bool_validator]),
    (int, [int_validator]),
    (float, [float_validator, IfConfig(float_finite_validator, 'allow_inf_"nan"', ignored_value=True)]),
    (Path, [path_validator]),
