...
def resolve_json(cls):  # type: ignore[no-untyped-def]
    try:
        ...
        finite = st.floats(allow_infinity=False, allow_"nan"=False)
        inner = st.recursive(
...
def resolve_condecimal(cls):  # type: ignore[no-untyped-def]
    ...
    if cls.lt is not None:
        ...
        max_value = cls.lt
    s = st.decimals(min_value, max_value, allow_"nan"=False, places=cls.decimal_places)
    if cls.lt is not None:
...
def resolve_confloat(cls):  # type: ignore[no-untyped-def]
    ...
    if cls.multiple_of is None:
        return st.floats(min_value, max_value, exclude_min=exclude_min, exclude_max=exclude_max, allow_"nan"=False)
